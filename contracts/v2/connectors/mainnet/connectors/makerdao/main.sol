...
import { TokenInterface, AccountInterface, InstaMappingInterface } from "../../common/interfaces.sol";
...
abstract contract MakerResolver is Helpers, Events {
  ...
  * @param colType Type of Collateral.(eg: 'ETH-A')
  ...
  function open(string calldata colType) external payable returns (string memory _eventName, bytes memory _eventParam) {
    ...
    InstaMappingInterface instaMapping = getInstaMapping();
  ...
  * @param setId ID stores the amount of tokens deposited.
  ...
  address colAddr = getInstaMapping().gemJoinMapping(ilk);
...
_eventParam = abi.encode(_vault, ilk, _amt, getId, setId);
  ...
  */
    ...
    * @param getId ID to retrieve amt.
    ...
    function getInstaMapping()internal view returns(InstaMappingInterface _instaMapping){
      _instaMapping = InstaMappingInterface(getMemory().getBroadcastAddr(uint(keccak256("INSTAMAPPING"))));
      require(address(_instaMapping)!=address(0), "INSTAMAPPING address is unkown!");
    }
}
